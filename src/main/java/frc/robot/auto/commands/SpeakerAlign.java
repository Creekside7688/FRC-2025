...
public class SpeakerAlign extends Command {
    ...
    public SpeakerAlign(SwerveDrive swerveDrive, PhotonCamera camera, Supplier<Pose2d> poseProvider) {
    ...
    public void execute() {
        ...
        if(photonResults.hasTargets()) {
            ...
            Optional<PhotonTrackedTarget> filteredTag = photonResults.getTargets().stream()
                .filter(t -> (t.getFiducialId() == VisionConstants.RED_SPEAKER) || (t.getFiducialId() == VisionConstants.BLUE_SPEAKER)) // To find the tag ID we want
            ...
            if(filteredTag.isPresent()) {
                ...
                Pose2d goalPose = tagPose.transformBy(VisionConstants.SPEAKER_DISTANCE).toPose2d();
